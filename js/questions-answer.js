//creo mi array de objetos con preguntas y respuestas del juego

const questionsAnswer = [
  {
    pregunta: "¿Quién es el autor de Bohemian Rapsody?",
    respuesta: 3,
    opciones: ["Rolling Stones", "Metallica", "The Beatles", "Queen"],
  },

  {
    pregunta:
      "¿Qué estrella es reconocida como la artista femenina más vendida de todos los tiempos?",
    respuesta: 0,
    opciones: ["Maddona", "Cher", "Tina Turner", "Gloria Estefan"],
  },

  {
    pregunta:
      "¿Cuál es el único país que ha ganado 3 concursos de canciones de Eurovisión seguidos?",
    respuesta: 2,
    opciones: ["Francia", "España", "Irlanda", "Italia"],
  },

  {
    pregunta: "¿Quién es el Rey del Rock?",
    respuesta: 2,
    opciones: [
      "Michael Jackson",
      "Freddie Mercury",
      "Elvis Presley",
      "Bob Dyland",
    ],
  },

  {
    pregunta: "¿Quién es conocido como El Sol de México?",
    respuesta: 1,
    opciones: ["Juan Gabriel", "Luis Miguel", "Christian Nodal ", "José José"],
  },

  {
    pregunta: "¿Cuál es el nombre artístico de Robyn Fenty?",
    respuesta: 2,
    opciones: ["Robyn", "Beyoncé", "Rihanna ", "Regina Spektor"],
  },

  {
    pregunta: "¿Dónde fué descubierto Justin Bieber?",
    respuesta: 0,
    opciones: ["YouTube", "Spotify", "TikTok", "Instagram"],
  },

  {
    pregunta: "¿Cuál es la famosa banda de K-Pop?",
    respuesta: 0,
    opciones: ["BTS", "Exo", "Rosa Negra", "Bing Bang"],
  },

  {
    pregunta: "¿Cuál es el nombre artístico de José Monje Cruz?",
    respuesta: 3,
    opciones: ["Canelitas", "Farrukito", "El Pele", "Camarón"],
  },

  {
    pregunta:
      "¿Qué grupo de rock clásico era conocido por estar drogado durante sus primeros conciertos?",
    respuesta: 0,
    opciones: ["Rolling Stones", "Metallica", "The Beatles", "Queen"],
  },

  {
    pregunta: "¿A qué grupo español le dolía la cara de ser tan guapo?",
    respuesta: 0,
    opciones: ["Los Inhumanos", "Mecano", "Radio futura", "Estopa"],
  },

  {
    pregunta: "¿Cuál es el nombre de pila del compositor clásico Vivaldi?",
    respuesta: 3,
    opciones: ["David", "Joseph", "Mateo", "Antonio"],
  },

  {
    pregunta: "¿Cuál fué la última canción que cantó Bob Marley?",
    respuesta: 0,
    opciones: ["Redemption song", "War", "Jamming", "Soul Rebel"],
  },

  {
    pregunta: "¿Qué edad tenía Jim Morrison cuando murió?",
    respuesta: 2,
    opciones: ["35", "25", "27", "40"],
  },

  {
    pregunta: "¿Qué Beatle cruzó Abbey Road descalzo?",
    respuesta: 3,
    opciones: ["Pete", "Ringo", "John", "Paul"],
  },
];
